...
{
	int c = 0; // Индикатор того, что количество первых элементов-нулей превосходит n
	cout << "\nРезультат:" << endl;
	for (int i = 0; i < n - z; ++i) // Цикл для проходения по искомому массиву
	{
		for (j = 0; j < n; ++j) // Цикл для проходения по исходному массиву
		{
	...
	// Если количество запрошеных элементов не превосходит количества первых элем-ов-нулей в файле
	if (c == 0)
